...
$configOp =     "Configuration Applied"
...
# Applied configuration settings via registry
if (!(Test-Path $configPath)){
    New-Item $configPath -Force
}
New-ItemProperty -Path $configPath -Name "ContentCachePath" -Value "%LOCALAPPDATA%\Google\DriveFS" -PropertyType String -Force
New-ItemProperty -Path $configPath -Name "AutoStartOnLogin" -Value "1" -PropertyType DWord -Force
New-ItemProperty -Path $configPath -Name "DefaultMountPoint" -Value "G" -PropertyType String -Force
New-ItemProperty -Path $configPath -Name "DirectConnection" -Value "1" -PropertyType DWord -Force
